...
import type { IChessEngine, EngineMove, EngineInfo } from '../types';

/**
 * Fairy Stockfish Engine Adapter for Server
 * 
 * Implements UCI protocol communication with Fairy Stockfish.
 * Designed for pooled usage in server environment.
 */
export class FairyStockfishEngine implements IChessEngine {
  ...
  private parseMove(moveStr: string): EngineMove {
    // Handle null/no move cases
    if (!moveStr || moveStr === '(none)' || moveStr === '0000') {
      return { from: '(none)', to: '(none)' };
    }

    // Check for minimum length for valid moves
    if (moveStr.length < 4) {
      return { from: '(none)', to: '(none)' };
    }
